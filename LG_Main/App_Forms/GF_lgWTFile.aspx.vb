Partial Class GF_lgWTFile
  Inherits SIS.SYS.GridBase
  Private _InfoUrl As String = "~/LG_Main/App_Display/DF_lgWTFile.aspx"
  ...
  Protected Sub GVlgWTFile_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles GVlgWTFile.Init
    DataClassName = "GlgWTFile"
    SetGridView = GVlgWTFile
  End Sub
  ...
  Protected Sub Page_Init(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Init
    ...
    End If
		...
		Dim validateScriptDocPK As String = "<script type=""text/javascript"">" & _
			...
			"    validated_FK_LG_WTFile_DocPK_main = true;" & _
			"    validate_FK_LG_WTFile_DocPK(o);" & _
			"  }" & _
		  "</script>"
		...
		End If
		Dim validateScriptFK_LG_WTFile_DocPK As String = "<script type=""text/javascript"">" & _
			"  function validate_FK_LG_WTFile_DocPK(o) {" & _
			"    var value = o.id;" & _
			"    var DocPK = $get('" & F_DocPK.ClientID & "');" & _
			"    try{" & _
			"    if(DocPK.value==''){" & _
			"      if(validated_FK_LG_WTFile_DocPK.main){" & _
			...
			"    PageMethods.validate_FK_LG_WTFile_DocPK(value, validated_FK_LG_WTFile_DocPK);" & _
			"  }" & _
			"  validated_FK_LG_WTFile_DocPK_main = false;" & _
			"  function validated_FK_LG_WTFile_DocPK(result) {" & _
			...
			"  }" & _
		  "</script>"
		If Not Page.ClientScript.IsClientScriptBlockRegistered("validateFK_LG_WTFile_DocPK") Then
			Page.ClientScript.RegisterClientScriptBlock(GetType(System.String), "validateFK_LG_WTFile_DocPK", validateScriptFK_LG_WTFile_DocPK)
		End If
  End Sub
	<System.Web.Services.WebMethod()> _
  Public Shared Function validate_FK_LG_WTFile_DocPK(ByVal value As String) As String

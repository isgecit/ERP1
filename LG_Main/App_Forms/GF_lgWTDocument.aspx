<%@ Page Language="VB" MasterPageFile="~/MasterPage.master" AutoEventWireup="false" CodeFile="GF_lgWTDocument.aspx.vb" Inherits="GF_lgWTDocument" title="Maintain List: WT Documents" %>
...
<asp:UpdatePanel ID="UPNLlgWTDocument" runat="server">
  <ContentTemplate>
    <asp:Label ID="LabellgWTDocument" runat="server" Text="&nbsp;List: WT Documents" Width="100%" CssClass="sis_formheading"></asp:Label>
    <table width="100%"><tr><td class="sis_formview"> 
    <LGM:ToolBar0 
      ID = "TBLlgWTDocument"
      ToolType = "lgNMGrid"
      EditUrl = "~/LG_Main/App_Edit/EF_lgWTDocument.aspx"
      EnableAdd = "False"
      ValidationGroup = "lgWTDocument"
      Skin = "tbl_blue"
      runat = "server" />
    <asp:UpdateProgress ID="UPGSlgWTDocument" runat="server" AssociatedUpdatePanelID="UPNLlgWTDocument" DisplayAfter="100">
      <ProgressTemplate>
    ...
    <script type="text/javascript">
      ...
      function script_download(id, typ, id1) {
      	pcnt = pcnt + 1;
      	var nam = 'wdwd' + pcnt;
      	var url = self.location.href.replace('App_Forms/GF_lgWTDocument.aspx', 'App_Download/filedownload.aspx');
      	url = url + '?id=' + id + '&typ=' + typ + '&id1=' + id1;
      	window.open(url, nam, 'left=20,top=20,width=100,height=100,toolbar=1,resizable=1,scrollbars=1');
      	return false;
      }
    </script>
    <asp:GridView ID="GVlgWTDocument" SkinID="gv_silver" BorderColor="#A9A9A9" width="100%" runat="server" AllowPaging="True" AllowSorting="True" AutoGenerateColumns="False" DataSourceID="ODSlgWTDocument" DataKeyNames="DocPK">
      <Columns>
        <asp:TemplateField>
          <ItemTemplate>
						<Table><tr>
        ...
        <asp:TemplateField HeaderText="DocPK" SortExpression="DocPK">
          <ItemTemplate>
    ...
    <asp:ObjectDataSource 
      ...
      SelectMethod = "UZ_lgWTDocumentSelectList"
      TypeName = "SIS.LG.lgWTDocument"
      SelectCountMethod = "lgWTDocumentSelectCount"
  ...
  <Triggers>
    <asp:AsyncPostBackTrigger ControlID="GVlgWTDocument" EventName="PageIndexChanged" />

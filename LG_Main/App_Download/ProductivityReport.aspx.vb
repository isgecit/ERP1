...
Public Class ProductivityReportClass
  ...
  Public Shared Function GetProductivityReport(ByVal FromDate As String, ByVal ToDate As String, ByVal Division As String) As List(Of ProductivityReportClass)
    ...
    ToDate = ToDate.Substring(6, 4) & "-" & ToDate.Substring(3, 2) & "-" & ToDate.Substring(0, 2)
    Dim FilterActivity As String = "(1,2,75,77)"
    Dim FilterGroup As String = "('ENGG001','ENGGC','ENGGD','ENGGF','ENGG005','ENGG002','ENGG003','ENGGE','ENGG004')"
    Dim VaultDB As String = "BOILER"
    Select Case Division
      Case "PUNE"
        FilterActivity = "(1,2)"
    ...
    Sql = Sql & "       from ttiisg910200 hh "
    Sql = Sql & "       where hh.t_acid in " & FilterActivity
    Sql = Sql & "         and substring(aa.t_docn,1,6) = ltrim(hh.t_cprj)"
    Sql = Sql & "         and substring(aa.t_docn,8,8) = ltrim(hh.t_cspa)"
    Sql = Sql & "         and substring(aa.t_docn,17,3) = ltrim(hh.t_dcat) "
    Sql = Sql & "         and ( substring('0000'+ltrim(substring(aa.t_docn,21,4)) ,len('0000'+ltrim(substring(aa.t_docn,21,4)))-3,4)"
    Sql = Sql & "           = substring('0000'+ltrim(hh.t_dsno),len('0000'+ltrim(hh.t_dsno))-3 ,4)   "
    Sql = Sql & "             )"
    Sql = Sql & "         and hh.t_tdat<=aa.t_isdt  "
    Sql = Sql & "         and hh.t_grcd in " & FilterGroup
    Sql = Sql & "         ) as Hours, "
    Sql = Sql & "        (select top 1 hh.t_grcd    "
    Sql = Sql & "            from ttiisg910200 hh "
    Sql = Sql & "		         where(substring(aa.t_docn, 1, 6) = LTrim(hh.t_cprj))"
    Sql = Sql & "              and substring(aa.t_docn,8,8) = ltrim(hh.t_cspa)"
    Sql = Sql & "              and substring(aa.t_docn,17,3) = ltrim(hh.t_dcat) "
    Sql = Sql & "              and ( substring('0000'+ltrim(substring(aa.t_docn,21,4)) ,len('0000'+ltrim(substring(aa.t_docn,21,4)))-3,4)"
    Sql = Sql & "                    = substring('0000'+ltrim(hh.t_dsno),len('0000'+ltrim(hh.t_dsno))-3 ,4)   "
    ...
    Sql = Sql & " and   substring(aa.t_docn,1,20)+ substring('0000'+ltrim(substring(aa.t_docn,21,4)) ,len('0000'+ltrim(substring(aa.t_docn,21,4)))-3,4)   "
    Sql = Sql & "      in (select ltrim(hh.t_cprj)+'-'+ltrim(hh.t_cspa)+'-'+ltrim(hh.t_dcat)+'-'+substring('0000'+ltrim(hh.t_dsno),len('0000'+ltrim(hh.t_dsno))-3 ,4)    "
    Sql = Sql & "                    from ttiisg910200 hh "
    Sql = Sql & "                    where hh.t_acid in " & FilterActivity
    Sql = Sql & "                      and hh.t_tdat<=aa.t_isdt  "
